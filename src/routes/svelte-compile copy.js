...
const scoped = (fn) => {
  const scoped_fn = (node2, state) => {
    return fn(node2, __spreadProps(__spreadValues({}, state), {
...
const handle_var_declaration = (node2, state) => {
  ...
  const declarators = node2.declarations.map((d) => handle(d, __spreadProps(__spreadValues({}, state), {
...
const parse$1 = (source, opts) => {
  ...
  const ast = parse3(source, __spreadValues({ onComment }, opts));
...
const parseExpressionAt$1 = (source, index, opts) => {
  const comments = [];
  const { onComment, enter, leave } = get_comment_handlers(comments, source);
  const ast = parseExpressionAt2(source, index, __spreadValues({ onComment }, opts));
  walk(ast, { enter, leave });
  return ast;
};
...
function svelte_compile(code) {
  return compile(code);
}
export { svelte_compile };
